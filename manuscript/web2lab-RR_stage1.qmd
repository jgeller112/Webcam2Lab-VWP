...
-   High-quality condition: A Logitech Brio webcam will record in 4K resolution (up to 4096 × 2160 px) with a 90° field of view, providing high-fidelity video suitable for accurate gaze estimation and pupil tracking.
...
For eye-tracking preprocessing we will use the {webgazeR} package in R that contains helper functions to preprocess webcam eye-tracking data. All webcam eye-tracking files will be merged. Data quality will be screened via sampling-rate checks with very low-frequency recordings (e.g., \< 5 Hz) by subject and trial excluded (**Bramlett & Wiener, 2024)**. We will quantify out-of-bounds (OOB) samples—gaze points outside the normalized screen (1,1)—and remove participants and trials with excessive OOB data ( \> 30%). OOB samples will be discarded prior to analysis. In addition, Gorilla provides calibration/quality metrics (“convergence” and “confidence,” both 0–1); trials with convergence \< 0.5 or confidence \> 0.5 will be excluded.
...
To analyze overall competition effects and onset latency, we will use generalized additive mixed models (GAMMs; Wood, 2017). GAMMs extends the generalized linear modeling framework by modeling effects that are expected to vary nonlinearly over time–a common feature in the VWP [@brown-schmidt2025; @mitterer2025; @verissimo2025novel]. These models capture nonlinear effects by fitting smoothing splines—or “wiggles”—to the data using data-driven, machine-learning-based methods. This approach reduces the risk of under and over-fitting and eliminates the need to prespecify polynomial terms, as required in traditional growth curve models [@mirman]. Importantly, GAMMs also allow researchers to account for autocorrelation in time-series data, which is especially critical in gaze analyses where successive samples are not independent. By modeling the autocorrelation structure, GAMMs provide more accurate estimates of temporal effects and prevent inflation of Type I error rates. In addition to this, fitting GAMMs allow us to estimate the onset of the competition effect in each condition [see @verissimo2025novel].

Gaze samples will be analyzed with a binomial (logistic) GAMM using the `bam()` function from the **{mgcv}** package [@Wood2017]. For visualization, we will employ the `plot_smooth()` and `plot_diff()` functions from the **{itsadug}** package [@itsadug], and use the **{onsets}** package [@verissimo2025novel] to examine onset latencies. The dependent variable consisted of gaze counts to cohort and unrelated pictures, for each participant and in each 100 ms time bin. All analyses were conducted on a window ranging from -100 ms from target onset to 1200 ms.

The model will include fixed effects of webcam type (effects-coded: high = 0.5, low = –0.5) and item type (effects-coded: cohort = 0.5, unrelated = –0.5), as well as their interaction, capturing the overall (parametric) effects. To examine how webcam type modulates the cohort effect over time, these two factors will be combined into a single four-level factor. Nonlinear, time-dependent effects will be modeled using factor smooths for time and for time-by-condition interactions, with condition treated as a categorical variable. This specification allows the model to capture both the overall differences in fixation proportions between conditions (via the parametric component) and the dynamic, time-varying patterns unique to each condition (via the smooth terms), while appropriately accounting for within-subject dependencies. We will first fit an initial model to estimate the autocorrelation parameter, which will then be included in the final model. Although using larger time bins reduces autocorrelation, it does not eliminate it entirely, and we aim to ensure that any remaining temporal dependencies are properly modeled.

::: callout-important
Why dont we model random wiggliness bs="fs"?)
:::
...
              s(time, by = condfactor, k = 10) +      # condition-specific curves interaction

              s(participant, bs = "re") +        # random intercepts

              s(time, participant, by=conditionfactor,  bs = "re"),  # subject smooths
...
          discrete = TRUE, rho = rho, AR.start = dat$AR.start,

          data = dat, na.action = na.omit, select = TRUE)
...
                          time_var = "time",           # Name of time variable
