...
All tasks were completed in a single session lasting approximately 30 minutes. The experiment was programmed and administered in Gorilla (Anwyl-Irvine et al., 2020) **and run using the Google Chrome browser**. Participants were tested in a dedicated room in the Human Neuroscience Lab at Boston College and were seated approximately 65 cm from a 23-inch Dell U2312HM monitor (1920 × 1080 px). Each testing computer was connected to the network via a wired ethernet connection. Auditory information was presented over Sony ZX110 headphones to ensure consistent audio delivery and to minimize background noise. Experimental tasks were fixed and presented in the following order: informed consent, the single-word visual world paradigm (VWP) task, and a demographic questionnaire. The complete experiment is available for viewing on the Gorilla platform.

After consenting participants will start the main VWP task. They will watch an instructional video demonstrating the calibration procedure (available on OSF here: Calibration will be completed twice—once at the beginning of the experiment and again after 30 trials. **Each calibration session allows up to three attempts. During calibration, participants complete a passive calibration task in which nine red targets are presented sequentially. Participants are instructed to look directly at each target while it is on the screen (\~ 200 ms with 10 predictions points with 1000 ms transition time). Immediately after, validation is performed using five green targets presented one at a time; participants are again instructed to look directly at each target. During validation, the eye-tracking system evaluates calibration quality by comparing the predicted gaze location to the known target location. Calibration is considered unsuccessful if more than 2 points deviate from the intended target location (i.e., predictions are missaligned to another calibration point).**

Participants will then complete four practice trials to familiarize themselves with the task. Each trial begins with a 500 ms central fixation cross, followed by a preview display of four images located in the screen’s corners. After 1500 ms, a start button appears at the center; participants click it to confirm fixation before hearing the spoken word. The images remain visible throughout the trial, and participants indicate their response by clicking the image corresponding to the spoken target. A response deadline of 5 seconds will be used. **Eye movements will be recorded continuously during the final image display.** Please see Fig. X for a schematic of the trail. After the main task, participants complete a brief demographic questionnaire and are thanked for their participation.
...
For analysis, gaze data will be binned into 100-ms time windows (effectively downsampling to 10 Hz). Within each bin, looks will be classified as directed toward either the cohort or the unrelated item. For each participant × trial × time bin, we will count the number of gaze samples falling on the cohort and the unrelated item. If the majority of samples within a bin fall on the cohort, that bin will be coded as 1; otherwise, it will be coded as 0. These binary values will then be summed across trials, yielding a dataset indexed by participant × time bin for each webcam recording. The resulting cohort-look counts serve as the dependent measure for statistical modeling and summary analyses.
