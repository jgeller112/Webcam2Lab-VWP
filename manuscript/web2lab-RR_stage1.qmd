...
To analyze overall competition effects and onset latency, we will use generalized additive mixed models (GAMMs; Wood, 2017). GAMMs extends the generalized linear modeling framework by modeling effects that are expected to vary nonlinearly over time–a common feature in the VWP and eye-tracking data/inetsnive longitidudnianl data in general (Brown-Schmidt et al., 2025; Hotteiger, 2025’ Veríssimo & Sol, 2025). These models capture nonlinear effects by fitting smoothing splines—or “wiggles”—to the data using data-driven, machine-learning-based methods. This approach reduces the risk of overfitting and eliminates the need to prespecify polynomial terms, as required in traditional growth curve models (Mirman, 2012). Importantly, GAMMs also allow researchers to account for autocorrelation in time-series data, which is especially critical in gaze analyses where successive samples are not independent. By modeling the autocorrelation structure, GAMMs provide more accurate estimates of temporal effects and prevent inflation of Type I error rates. In addition to this, fitting GAMMs allow us to estimate the onset of the competition effect in each condition. 

Gaze samples will be analyzed with a binomial (logistic) GAMM using the `bam()` function from the **{mgcv}** package (Wood, 2023). For visualization, we will employ the plot_smooth() and plot_diff() functions from the **{itsadug}** package (Rij et al., 2022), and use the **{onsets}** package to examine onset latencies. The dependent variable consisted of gaze counts to  cohort and unrelated pictures, for each participant and in each 100 ms time bin. All analyses were conducted on a window ranging from -100 ms from target onset to 1200 ms.

The model will include fixed effects of webcam type (effects-coded: high = 0.5, low = –0.5) and item type (effects-coded: cohort = 0.5, unrelated = –0.5), along with their interaction, capturing the overall (parametric) effects. To model how webcam type modulates the cohort effect over time, the two factors will be combined into a single four-level factor. Nonlinear, time-dependent effects will be modeled using factor smooths for time and for time-by-condition interactions, with condition treated categorically. This specification allows the model to capture both the overall differences in fixation proportions between conditions (via the parametric component) and the dynamic temporal patterns unique to each condition (via the smooth terms), while properly accounting for within-subject dependencies. Below is some sample model syntax we will use to fit the data. Note that we will be fitting an intiral model to get autocorrelation for the model. Although larger timebins suffer less from autocorrelation, it does not entirely get rid of it, and we want to make sure we capture it in the model.
...
To examine whether webcam affects calibration rejection, we will fit a logistic regression model using the glm function. 

```{r}
glm(calibration ~ webcam, family = binomial(link = "logit"))
```

# Experiment 2: Chin rest vs. no chin rest

In Experiment 2, using the high quality webcam, we will examine how participant head movement affects competition effects.  We hypothesize several effects related to competition, onset, and attrition. **Competition Effects:** (H1a) Participants will show a competition effect, with more looks directed toward cohort competitors than unrelated distractors. (H1b) Use of a chin rest (vs. no chin rest) is not expected to influence the overall proportion of looks. However, (H1c) the magnitude of the competition effect is predicted to be larger in the chin rest condition than in the no chin rest condition. **Onset Effects:** (H2a) Looks to cohort competitors will emerge earlier than looks to unrelated distractors. (H2b) The onset of looks is expected to occur earlier in the chin rest condition than in the no chin rest condition. (H2c) Accordingly, the competition effect should emerge sooner in the chin rest condition compared to the no chin rest condition. **Attrition:** (H3) Attrition rates are expected to be lower in the chin rest condition compared to the no chin rest condition.
