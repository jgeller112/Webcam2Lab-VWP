...
keywords: [webcam eye-tracking,  webcameras, Visual world paradigm,  Lab-based experimentation, competition, spoken word recognition]
authornote: |
...
All stimuli (audio and images), code, and data (raw and summary) will be placed on OSF at this link: <https://osf.io/cf6xr/overview>. The entire experiment will be stored on Gorilla's open materials with a link to preview the tasks. In addition, the code and manuscript will be fully reproducible using Quarto and the package manager nix [@nix] in combination with the R package {rix} [@rix] . Together, nix and {rix} enable reproducible computational environments at both the system and package levels. This manuscript and all of the necessary files to reproduce it will be stored on GitHub. **Participants will provide informed consent prior to participation. At the time of data collection, all study procedures have been approved by the relevant ethics committee.**

## Sampling Goal

**We conducted an *a priori* power analysis using the mixedPower package** [@kumle2021] **in R . Data from 21 participants, collected online using the Gorilla experimental platform during development of the {webgazeR} \[@gellerLanguageBordersStepbystep2025f\] package and employing the same stimuli and visual-world word paradigm (VWWP) design, were used to seed the simulations. In these data, we observed a cohort effect of approximately 3%. Using this value as the effect-size seed, we collapsed the data across time bins to compute binomial fixation counts per trial and fit a binomial generalized linear mixed-effects model (GLMM) to obtain fixed-effect estimates. Because the planned experiment included a manipulation of webcam quality, we conducted simulations incorporating this factor. Specifically, we augmented the dataset with a between-subjects factor for webcam quality, with participants evenly assigned to high-quality and standard-quality webcam groups. In the high-quality webcam group, we modeled both a higher overall fixation rate and a larger cohort effect. In the standard-quality webcam group, the cohort effect was reduced by half relative to the high-quality group.**
...
To manipulate recording quality, two webcams will be used. In the high-quality condition, we will use a Logitech Brio webcam, which records in 4K resolution (up to 4096 × 2160 px) with a 90° field of view and samples at 60 Hz. This setup provides high-fidelity video with greater spatial and temporal precision. In the standard-quality condition, we will use a Logitech C270 HD webcam, which records in 720p resolution and samples at 30 Hz, producing video quality comparable to that of a typical built-in laptop webcam and therefore simulating lower-quality online recordings [@jarvis2025].
...
All tasks were completed in a single session lasting approximately 30 minutes. The experiment was programmed and administered in Gorilla (Anwyl-Irvine et al., 2020) **and run in the Google Chrome browser**. Participants were tested in a dedicated room in the Human Neuroscience Lab at Boston College with **two computers** and were seated approximately 65 cm from a 23-inch Dell U2312HM monitor (1920 × 1080 px). Each testing computer was connected to the same network via a wired ethernet connection (**internet speed was tested before each session to ensure a high-speed connection)**. Auditory information was presented over Sony ZX110 headphones to ensure consistent audio delivery and to minimize background noise.

Experimental tasks were fixed and presented in the following order: informed consent, the single-word visual world paradigm (VWP) task, and a demographic questionnaire. The complete experiment is available for viewing on the Gorilla platform.
...
**After calibration**, participants will then complete four practice trials to familiarize themselves with the task. Each trial begins with a 500 ms central fixation cross, followed by a preview display of four images located in the screen’s corners. After 1500 ms, a start button appears at the center; participants click it to confirm fixation before hearing the spoken word. The images remain visible throughout the trial, and participants indicate their response by clicking the image corresponding to the spoken target. A response deadline of 5 seconds will be used. **Eye movements will be recorded continuously during the final image display.** Please see Fig. X for a schematic of the trail. After the main task, participants complete a brief demographic questionnaire and are thanked for their participation.

### **Demographic Questioniare**
...
**For analysis, gaze data or looks were binned into 100-ms time windows (10 Hz). Within each bin, gaze samples falling on the target item were excluded. Remaining looks were binarised as either falling on either the cohort competitor or one of the unrelated competitors, with looks to either unrelated item collapsed into a single unrelated category. For each participant × trial × time bin, we counted the number of looks on the cohort and on unrelated items. Each bin was then coded as cohort-dominant (1) if the cohort received more samples and as 0 otherwise. These binary values were summed across trials, yielding for each participant × time bin the number of trials in which gaze favored the cohort over unrelated competitors. This cohort-dominance count served as the dependent measure for statistical modeling and summary analyses. This simplified analysis has a several advantages: 1. It is easier to analyze data due to reduced dimensoniality, and as well as reduced autocorrelation [@verissimo2025novel].** Ina gamm framework binomial distriction families allow one to model autorocrrection directly which we do (see below).
...
In Gorilla, calibration failure occus when the centroid of x, y is near another point.
...
(H3) **Attrition due to calibration failures** will be lower in the high-quality webcam condition than in the standard-quality webcam condition.
...
I conset for my paper to be publihsed in BRM.
...
All data, code, and materials will be stored on OSF (https://osf.io/cf6xr/overview) and GitHub (https://github.com/jgeller112/Webcam2Lab-VWP).
